...
$postgresHealthy = Wait-ForService -Name "PostgreSQL" -Url "http://localhost:5432" -MaxWaitSeconds 60
if (-not $postgresHealthy) {
    Write-Host "PostgreSQL health check failed. Check logs with: docker compose logs postgres" -ForegroundColor Red
    exit 1
}

$redisHealthy = Wait-ForService -Name "Redis" -Url "http://localhost:6379" -MaxWaitSeconds 30
if (-not $redisHealthy) {
    Write-Host "Redis health check failed. Check logs with: docker compose logs redis" -ForegroundColor Red
    exit 1
}

$qdrantHealthy = Wait-ForService -Name "Qdrant" -Url "http://localhost:6333/healthz" -MaxWaitSeconds 60
if (-not $qdrantHealthy) {
    Write-Host "Qdrant health check failed. Check logs with: docker compose logs qdrant" -ForegroundColor Red
    exit 1
}
