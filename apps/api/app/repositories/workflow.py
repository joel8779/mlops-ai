...
class WorkflowRepository(BaseRepository[CandidatePipelineStage]):
    ...
    async def current_stage(self, candidate_id: UUID, job_description_id: UUID | None) -> CandidatePipelineStage | None:
    ...
    async def stage_counts(self, organization_id: UUID) -> dict[str, int]:
        result = await self.db.execute(
            select(CandidatePipelineStage.stage, func.count())
            .where(CandidatePipelineStage.organization_id == organization_id)
...
class RecruiterActivityRepository(BaseRepository[RecruiterActivity]):
    ...
    async def timeline(self, organization_id: UUID, limit: int = 50) -> list[RecruiterActivity]:
        result = await self.db.execute(
            select(RecruiterActivity)
            .where(RecruiterActivity.organization_id == organization_id)
